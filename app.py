...
def on_message(client, userdata, msg):
    ...
    try:
        data = json.loads(msg.payload.decode())
        temp, hum, fan = data.get("temp"), data.get("hum"), data.get("fan", 0)
        last_data = {"temp": temp, "hum": hum, "fan": fan}
        save_row(temp, hum, fan)
        socketio.emit("update", last_data)
    except Exception as e:
        print("[MQTT] Error:", e)
...
@app.route("/mqtt_status")
def mqtt_status():
    return jsonify({"connected": mqtt.is_connected(), "broker": MQTT_HOST, "port": MQTT_PORT})
